...
- For Docker Hub-origin images, try the pinned `docker.sparkcr.cn/<upstream-image>` accelerator before direct Docker Hub. Treat `https://status.anye.xyz/` as an operator discovery page only: review a candidate, pin it in code, and verify the pulled manifest/digest before use; never execute or trust a dynamically returned mirror list at runtime.
