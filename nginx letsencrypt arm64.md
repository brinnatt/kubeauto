...
+ 宿主机 CPU 架构为 **ARM64**（aarch64 / arm64v8）。Docker 会拉取对应架构的镜像；本文 Certbot 使用 `certbot/certbot:arm64v8-v1.32.0` 标签，mysql、wordpress、nginx 官方镜像均提供多架构 manifest，在 ARM64 上会自动匹配 arm64 层。
...
注意，配置中已使用生产域名 `brinnatt.com`；若自行部署，请将 `server_name` 等处的域名替换为自己的。该 server 代码块的核心指令如下：
...
+ `server_name`：声明本 `server` 块处理的虚拟主机名；请求中的 `Host` 头与之匹配时由该块处理。可写多个名字，第一个通常视为主名。
...
在项目的根目录 `/usr/local/wordpress` 下编辑 `.env` 文件：
...
+ image：使用 `wordpress:6.1.0-fpm-alpine`。`-fpm` 表示内置 **PHP-FPM**（FastCGI 进程管理器），由 Nginx 通过 FastCGI 协议转发 PHP 请求；`alpine` 基于 [Alpine Linux](https://alpinelinux.org/)，镜像体积更小。官方多架构镜像在 ARM64 宿主机上自动拉取 arm64 层。
...
该定义拉取 `certbot/certbot:arm64v8-v1.32.0` 镜像。ARM64 平台上 Certbot 官方提供 `arm64v8` 标签变体（参见 [certbot/certbot on Docker Hub](https://hub.docker.com/r/certbot/certbot/tags)）；固定版本号 `v1.32.0` 避免 `latest` 漂移。Certbot 分两类插件（参见 [Certbot 用户指南](https://eff-certbot.readthedocs.io/en/stable/using.html)）：
...
输出中 `Successfully received certificate` 表示生产 CA 已签发正式证书；`Account registered` 表示首次向 Let's Encrypt 注册了 ACME 账户。webroot + `certonly` 模式下不会出现 `Plugins selected: Authenticator webroot, Installer None` 之外的 Installer 行为——Nginx SSL 仍由下文手写配置完成。
...
HTTPS `server` 块要点：
+ `listen 443 ssl http2`：在 443 启用 SSL/TLS；`http2` 作为 `listen` 参数在 nginx 1.25.1+ 已弃用，应改用独立 `http2 on` 指令；本文使用 nginx 1.22.1，此写法正确且与当时官方示例一致。
...
该脚本设置 Compose 与 Docker 可执行文件路径；`--ansi never`（旧版 Compose 为 `--no-ansi`）避免 cron 邮件中出现 [ANSI 控制字符](https://vt100.net/docs/vt510-rm/chapter4.html)。`cd` 进入项目目录 `/usr/local/wordpress` 后执行：
