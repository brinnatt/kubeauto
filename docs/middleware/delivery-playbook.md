...
文档导航不采用人工抽查作为唯一保障。`tests/unit/test_middleware_documentation.py` 从
`docs/middleware/` 的实际组件目录计算组件集合，并同时校验以下关系：

```mermaid
flowchart LR
    D[组件目录] --> M[三份标准手册]
    M --> I[docs/middleware/README.md]
    M --> R[根 README.md]
    I --> S[状态字段]
    R --> S
    S --> G[独立分路静态门禁]
...
每个组件目录必须同时包含 `operations-manual.md`、`technical-whitepaper.md` 和
`development-manual.md`。中间件索引和根 README 必须分别为每个目录提供完整链接，且
状态字段必须一致并使用受控状态词。索引还必须链接当前专项矩阵；状态为“已交付”时，
矩阵结果必须为 `PASS`。总交付运行器在进入任何核心或独立中间件分路、远程同步或集群
变更前都会执行该契约测试；因此新增目录、漏写入口、缺少手册、状态与矩阵不一致或删除
门禁，均会在最便宜的静态阶段失败。

新增组件的最小步骤为：创建标准文档目录，补齐中间件索引和根 README，创建专项矩阵
与交付测试，并运行 `bash tests/run_unit_tests.sh`。在文档门禁通过前，不得启动现场
部署、性能测试或修改矩阵为 `pass`。组件状态只在对应矩阵、供应链和清理证据完成后
更新为“已交付”。
